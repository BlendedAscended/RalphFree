...
| Rule Shape (added 2026-08-21) | A rule that records only the symptom gets obeyed literally and defeated anyway. "Never do A" is |
...
| Session Context Save (Obsidian) — standing rule, every session | After any session with durable decisions or multi-step work, save a context note to the Obsidian vault: |
...
| Gemini-Specific | (no summary available) |
| Projects | (no summary available) |
| Canonical Paths (HARD RULE) | When the user says "vflow2.0", "vflow", "veldonlab.com", or "Veldon Lab 2.0": |
| Conventions | (no summary available) |
| Quick Commands | # Start oMLX server |
| Obsidian Vault (Second Brain) | Location: ~/Documents/Obsidian Vault |
