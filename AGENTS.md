...
<!-- header-block:start -->
<!-- AUTO-GENERATED by sync-rules.py — DO NOT EDIT BY HAND -->

| Section | Summary |
|---|---|
| Who I Am | Name: San Ghotra |
| Current Projects (16 active) | [[RT]], [[Refree]], [[Sanctuary]], [[Aqwa]], [[Galaxy-Products]], [[Jack]], [[Gibby]], [[Zenith]], [[TrustOneServices]] |
| How This Vault Works | Vault Root |
| What I Want From AI Tools | (no summary available) |
| Behavioral Contract — 12 Rules | After each Weekly Synthesis (Monday 8am), Hermes reviews the 12 rules against the vault's work log: |
| Formatting Hard Stops | These apply to ALL output across ALL tools. No exceptions. |
| Safety Rules — Always Apply | Interpret "do not delete without permission" as protecting durable user/project data, not temporary caches or rebuild... |
| Agent Write Rules | (no summary available) |
| Concurrent Agent Protocol (ALWAYS ACTIVE — AUTOMATIC) | San runs multiple AI tools simultaneously: OpenCode, Claude Code, Codex, Hermes, VS Code Copilot, Gemini CLI, OpenCla... |
| Cross-Tool Architecture | Cross-tool config: |
| Voice and Style | Load and apply sandeep-universal-style on every response without exception. It governs formatting, banned patterns, t... |
| Scope Routing | Apply only rules relevant to the active task. No scope bleed. |
| Scope Discipline | (no summary available) |
| Skill Auto-Triggers | Load without being asked. |
| Memory Protocol | Session start: Read ~/.claude/memory/MEMORY.md. Load topic files relevant to the task domain. If MEMORY.md is absent:... |
| Changelog Rule | After any code change, feature, bug fix, or architecture update, run: |
| Verification (Auto-Run After Edits) | (no summary available) |
| Knowledge & Context | (no summary available) |
| Integration Points | (no summary available) |
| Merged Learnings from Projects | Apply docs/ai/CODE-STANDARDS.md. That file is the source of truth for: |
| Maintenance | (no summary available) |
| Instruction Hierarchy | Context imports (load on session start): |
| Cross-Tool Context Sharing | Shared files (one edit propagates to all): |
| Second Brain | Vault: ~/Documents/Obsidian Vault (OBSIDIAN_VAULT_PATH env var). |
| Memory Layer | Three-tier memory model. All layers are additive. L0 is never modified or trimmed by this section. |
| Code Style | (no summary available) |
| vflow2.0 Deployment (Hetzner VPS) | (no summary available) |
| Stack | (no summary available) |
| Key Paths | (no summary available) |
| Notes | (no summary available) |
| Obsidian Vault | Cross-tool knowledge base: ~/Documents/Obsidian Vault |
| 6. Reference Files | (no summary available) |
| OpenCode-Specific | (no summary available) |
...
- **Gate human approval by blast radius, not by blanket rule.** Agents act autonomously on internal, reversible, or read-only work and on Run-lane output the user completes themselves. Require approval (Telegram buttons or a repliable Slack thread, reply `approve / skip / edit`) only before irreversible, costly, or reputational external side effects where Veldon holds the keys (content publishing, Deploy-lane publishes, outbound client messaging, paid or committed actions). Failure alerts are visibility, not gates: on a failed or degraded run, notify and stop, never block on success. Do not re-add a human gate for work an agent has proven it handles safely on its own.
